                ...
                {imgs.map((img: { url: string; previewUrl?: string }, i: number) => (
                  <div key={i} className="flex-shrink-0 w-full flex items-center justify-center bg-surface-container-highest/20 animate-pulse select-none min-h-[380px] md:min-h-[520px]">
                    <img
                      ...
                      alt=""
                      className="w-full h-auto max-h-[450px] md:max-h-[650px] object-contain cursor-pointer hover:scale-[1.01] transition-transform duration-700 pointer-events-none opacity-0 transition-opacity"
                      referrerPolicy="no-referrer"
                      onLoad={(e) => {
                        e.currentTarget.classList.remove('opacity-0');
                        const parent = e.currentTarget.parentElement;
                        if (parent) {
                          parent.classList.remove('animate-pulse', 'bg-surface-container-highest/20', 'min-h-[380px]', 'md:min-h-[520px]');
                          parent.style.minHeight = '0';
                        }
                      }}
                      onError={(e) => {
